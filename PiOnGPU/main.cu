...
#define PRECISION_HEX 1008	//of digits in hex
#define PRECISION_OUTPUT_START 1000
#define PRECISION_OUTPUT_END 1008

#define WARP_SIZE 32
#define WARPS_PER_BLOCK 32	//1024 threads per block
...
__device__ void DecToHexModDiv(unsigned char dec, char &hex, char* hex_table)
{
	hex = hex_table[dec];
}
	...
	char *hex_table)
{
	...
	for (__int64 i = precision_current; i < precision_current + dLeap; i++)
	{
		...
		DecToHexModDiv(result_dec[i], result_hex[i], hex_table);
	}
}
...
int main(int argc, char **argv)
{
	...
	PiOnGPU_Kernel<<<BLOCKS_NUM, THREADS_PER_BLOCK>>>(PRECISION_INIT, PRECISION_HEX, dLEAP, result_dec_gpu, result_hex_gpu, hex_table);
